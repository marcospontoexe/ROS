...
class NavStack(object):
    ...
    def navigate(self):
        if not self.iniciou:    #executa apenas no início da execução do código
            ...
            print("main_global_planner_linear_dinamica_demo.py pronto.")
            nav_stack_object.log_manager.gera_log("main_global_planner_linear_dinamica_demo.py iniciado com sucesso.", LogManager.Info)
        ...
        #Determines if the robot has arrived or not  
            ...
            self.navegando = False  
            self.navegando_pub.publish(False)
            rospy.loginfo("main_global_planner_linear_dinamica_demo detectou objetivo alcançado.")
...
if __name__ == '__main__':
    rospy.init_node("main_global_planner_linear_dinamica_demo_node", anonymous=True)
    rate = rospy.Rate(1)
    nav_stack_object = NavStack()
    print("Iniciando main_global_planner_linear_dinamica_demo.py...")
    nav_stack_object.log_manager.gera_log("Iniciando  main_global_planner_linear_dinamica_demo.py.", LogManager.Info)
