...
class NavStack(object):
    ...
    def __init__(self):
        ...
        # self.caminho_rota = "/home/ubuntu/catkin_ws/src/nav_hub/scripts/config/demonstration_2pontos.json"
    ...
    def navigate(self):
        if not self.iniciou:    #executa apenas no início da execução do código
            ...
            print("main_global_planner_mesh_dinamica.py pronto.")
            nav_stack_object.log_manager.gera_log("main_global_planner_mesh_dinamica.py iniciado com sucesso.", LogManager.Info)
        ...
        #Determines if the robot has arrived or not  
            ...
            #clear the map
            os.system(self.command)  
            rospy.loginfo("main_global_planner_mesh_dinamica detectou objetivo alcançado.")
            ...
            if not self.flagChegando:
                ...
                if math.sqrt((self.poseAtual[0] - self.goal_x)**2 + (self.poseAtual[1] - self.goal_y)**2) <= 0.5:   #verifica se a distancia euclidiana entre o robo e o destino é menor do que 50 cm
                    ...
                    self.max_vel_theta(0.15)      # para girar lentamente, não perdendo a orientação
...
if __name__ == '__main__':
    rospy.init_node("main_global_planner_mesh_dinamica_node", anonymous=True)
    rate = rospy.Rate(1)
    nav_stack_object = NavStack()
    print("Iniciando main_global_planner_mesh_dinamica.py...")
    nav_stack_object.log_manager.gera_log("Iniciando  main_global_planner_mesh_dinamica.py.", LogManager.Info)
