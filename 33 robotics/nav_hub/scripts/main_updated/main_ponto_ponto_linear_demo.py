...
import actions_move
# from actions_move import Actions_Robot
...
class NavStack(object):
    ...
    def __init__(self):
        ...
        # self.caminho_rota = "/home/ubuntu/catkin_ws/src/nav_hub/scripts/config/park_demo.json"
        ...
        self.actions = actions_move.Actions_Robot()
    ...
    def navigate(self):
        if not self.iniciou:    #executa apenas no início da execução do código
            ...
            print("Navegação ponto a ponto linear DEMO pronta.")
            self.log_manager.gera_log("Navegação ponto a ponto linear DEMO iniciada com sucesso." , LogManager.Info)
...
if __name__ == '__main__':
    rospy.init_node("main_ponto_ponto_linear_demo_node")
    rate = rospy.Rate(2)
    nav_stack_object = NavStack()
    print("Iniciando main_ponto_ponto_linear_demo.py...")
    nav_stack_object.log_manager.gera_log("Iniciando main_ponto_ponto_linear_demo.py", LogManager.Info)
