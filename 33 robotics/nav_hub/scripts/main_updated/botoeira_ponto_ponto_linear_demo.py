...
class botoeira():
    ...
    def button_pressed(self, destino):
        ...
        self.reached = False
        if not self.flagLora:
            self.destination_pub.publish(destino)
        else:   # chamado via botoeira lora
            self.flagLora = False
            self.destination_pub.publish(destino+10)
    ...
    def await_button(self):
        ...
        if not self.flagPararTempo:
            ...
            self.log_manager.gera_log("Iniciando botoeira_ponto_ponto_linear_DEMO_node: " + str(self.tempo_atual) + " segundos.", LogManager.Info)
        ...
        if self.main_started:   # quando o main_ponto_ponto inicia
            ...
            if self.destinoEnviado and self.reached:   #quando o robo chega ao destino
                ...
                # print(f"botão: {self.botao}") 
                self.button_pressed(self.botao)    # manda para o proximo ponto
            ...
            if GPIO.input(self.SEQ_BUTTON) == GPIO.HIGH:
                ...
                if self.reached and not self.has_started:   # enquanto o botão de start não for apertado
                    # self.displayOled.clear()  # Limpa o display oled
                    # self.displayOled.display_text("DESTINO:", str(destino))  # Exibe texto no display oled
                    self.button_pressed(self.botao) # envia o ponto de destino

if __name__ == '__main__':
    rospy.init_node("botoeira_ponto_ponto_demo_node")
    objt = botoeira()
    rate = rospy.Rate(5)
