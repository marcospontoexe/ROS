...
class NavStack(object):
    ...
    def __init__(self):
        ...
        self.caminho_rota = "/home/ubuntu/catkin_ws/src/nav_hub/scripts/config/ponto-ponto-mesh/"    
    ...
    def destination_callback(self, data):
        ...
        if self.beginning:  # apenas no inicio
            ...
            print(f"Posição inicial configurada a partir do arquivo {destino}.json")
    ...
    def navigate(self):
        if not self.iniciou:    #executa apenas no início da execução do código
            ...
            print("Navegação ponto a ponto mesh pronta.")
            self.log_manager.gera_log("Navegação ponto a ponto mesh iniciada com sucesso." , LogManager.Info)
        ...
        #Determines if the robot has arrived or not  
            ...
            #clear the map
            os.system(self.command)
            rospy.loginfo("main_ponto_ponto_mesh_node detectou objetivo alcançado.")
...
if __name__ == '__main__':
    rospy.init_node("main_ponto_ponto_mesh_node")
    rate = rospy.Rate(2)
    nav_stack_object = NavStack()
    print("Iniciando main_ponto_ponto_mesh.py...")
    nav_stack_object.log_manager.gera_log("Iniciando main_ponto_ponto_mesh.py", LogManager.Info)
