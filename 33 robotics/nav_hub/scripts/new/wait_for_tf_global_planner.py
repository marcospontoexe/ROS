    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            if self.listener.canTransform("map", "odom", rospy.Time(0)):
                ...
                rospy.loginfo("Iniciando move_base_global_planner...")
                self.iniciar_em_zero()
                subprocess.call(["roslaunch", "nav_hub", "move_base_global_planner.launch"])
                break
            else:
...
if __name__ == "__main__":
    rospy.init_node("wait_for_tf_global_planner_node", anonymous=True)
    objt = wait_for_tf()
    objt.run()
